...
if '__main__' == __name__:
	...
	for source, samples_source in cells_per_source.items():
		...
		for ti, sym in enumerate(expr['symbols']):
			if sym in genelist_first and not 'ERCC' in expr['IDs'][ti]:
				avgrpkm = numpy.mean([expr[s][ti] for s in samples])
				genelist_sources[source][sym] = avgrpkm
	...
	for p, sample, sample_i, cellsource in table_loader():
		...
		if ERCC_rpkmsum < 100:
			print sample, 'has too low ERCC RPKM sum:', ERCC_rpkmsum
			continue
			#raise Exception
		
		mRNA_rpkmsum = sum(expr[sample][gene] for gene in genes_i)
		
		# changes below (3 lines) from previous versions, before v11 
		genelistD = genelist_sources[cellsource] # before some of the relabeling/splitting
		rpkm_cutoff_sample = 20 * 2**o.xfor20rpkm/(mRNA_rpkmsum/ERCC_rpkmsum * ERCC_moleculenumber)
		genelist = set(gene for gene,avgrpkm in genelistD.items() if avgrpkm >= rpkm_cutoff_sample)
		...
		print sample, cellsource, xarr[cellsource][-1], yarr[cellsource][-1], rpkm_cutoff_sample
