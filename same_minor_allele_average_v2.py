...
def give_sharing(clone_samples, minor_alleles, expra):
	...
	for ai, allele_order in minor_alleles.items():
		...
		cells_with_minor_allele_or_both = sum(expra[s+minor_a][ai]>=minreads for s in clone_samples)
		gene = expra['symbols'][ai]
		if cells_with_minor_allele_or_both >= 1:
			num_shared[gene] = cells_with_minor_allele >= max(2, expressing_cells*0.9)
	return len(num_shared.values())
...
if '__main__' == __name__:
	...
	print p_crude, p_normal, excess_genes, clone_shared, numpy.mean(random_shared)
